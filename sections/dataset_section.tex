...
The collected patent-derived reference corpus contains 2,572 optical system designs assembled from the CodeV patent library ~\cite{codev}, which primarily consists of Japanese and Russian patents; Zemax Zebase library ~\cite{zemax_opticstudio}, predominantly featuring United States patents; and a public file exchange site for lens designs ~\cite{reiley_lens_design_exchange}, which contains designs from diverse sources submitted by the community. Each sequence file in the curated database records proper source attribution through one of: patent identifiers, textbook or journal publication citations, or the best available source record. 

Since the data points and files collected from all these different sources have different file formats, we standardize them to CodeV's digital lens file, known as a \texttt{.seq} file. These design files are then updated to handle any residual issues in the source artifact itself or that occur during the standardization process.
Furthermore, many of the source files contained generic glass codes or obsolete material labels rather than a real glass material from existing material catalogues. This necessitates at first the usage of CodeV \texttt{GLASSFIT} procedure to adjust the material labels and then re-optimization of the image plane location, to adjust for the updated materials. Once the design parameters of all the curated designs are finalized using the aforementioned steps, each design was analyzed for the following performance metrics:

{\color{red} Details of all the performance metrics to be included here}

All these design modifications and evaluation runs are conducted on LLIMAS ~\cite{Doyle2017LLIMAS} compute system using CodeV scripting.

Finally, each curated prescription retains its source attribution and corresponding sequence-file artifact.
