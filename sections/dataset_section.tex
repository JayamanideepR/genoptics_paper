...
All collected designs are passed through a common CodeV preparation workflow. Source artifacts are converted to CodeV's digital lens format, represented by a \texttt{.seq} file, where needed. When a source file contains generic glass codes or obsolete material labels rather than materials from available optical catalogs, the CodeV \texttt{GLASSFIT} procedure is used to update its material assignments. The image-plane location is then re-optimized where needed to account for any material changes. These source-corpus preparation steps and the subsequent evaluation runs are automated through CodeV scripts executed on the LLIMAS compute system~\cite{Doyle2017LLIMAS} and precede the shortlisting process described below.

Each prepared optical system is retained in two linked forms: the associated sequence-file artifact and a tabular prescription row. Within the tabular prescription, the optical system is represented as a sequence of surfaces with associated geometric and material parameters. Figure~\ref{fig:lens_param} illustrates the lens assembly and the parameters used to define this representation. For each active surface $j$, the table records:
\begin{enumerate}
